...
// List of popular movies for initial load and suggestions
const popularMovies = [
  'Inception', 'The Godfather', 'The Shawshank Redemption', 'Pulp Fiction',
  'Forrest Gump', 'The Matrix', 'Goodfellas', 'Interstellar', 'Dhamaal', 'pk', 'Goreyan nu Daffa Karo',
  'Carry on Jatta', 'Manje Bistre', 'Housefull', 'Best of Luck', 'Yaar Anmulle',
  'Parasite', 'Gladiator', 'Joker', 'Whiplash', 'The Prestige', 'Taarzan: The Wonder Car', 'Sholay',
  'Spider-Man: Into the Spider-Verse', 'Saving Private Ryan', 'The Green Mile'
];

// Levenshtein Distance function for finding string similarity
...
function App() {
  const [state, setState] = useState({
    s: "",
    results: [],
    selected: {}, // This state is crucial for the popup
  });
  const [suggestion, setSuggestion] = useState("");
  const apiurl = "http://www.omdbapi.com/?apikey=dfe6d885";

  // Effect for loading initial movies
  useEffect(() => {
  ...
  // Main search function with suggestion logic
  const search = (term) => {
  ...
  const handleSuggestionClick = (suggestedTerm) => {
    setState(prevState => ({ ...prevState, s: suggestedTerm }));
  ...
  return (
    <div className="App">
      ...
      <main>
        ...
        {/* This is the line that shows/hides the popup */}
